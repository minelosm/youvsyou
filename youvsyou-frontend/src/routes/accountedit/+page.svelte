<script>
    import { page } from "$app/stores";
    import { isAuthenticated, user, jwt_token } from "../../store";
    import axios from "axios";

    const api_root = $page.url.origin;

    let fitnessuser = {
        email: $user.email,
        birthDate: null,
        height: null,
        weight: null,
    };

    function editUser() {
        var config = {
            method: "put",
            url: api_root + "/api/fitnessuser/edit/" + $user.email,
            headers: { Authorization: "Bearer " + $jwt_token },
            data: fitnessuser,
        };

        axios(config)
            .then(function (response) {
                alert("User Information edited successfully");
            })
            .catch(function (error) {
                alert("Error editing user information");
                console.log(error);
            });
    }
</script>

<h1>Edit Account</h1>
<p>{$user.user_roles}</p>
{#if $isAuthenticated && $user.user_roles === "fitnesscoach"}
    <h2>Fullfillment Account Details</h2>

    <div class="mb-3">
        <label for="email" class="form-label">Email address</label>
        <input
            type="email"
            class="form-control"
            id="email"
            placeholder={$user.email}
            readonly
        />
    </div>
    <div class="mb-3">
        <label for="username" class="form-label">Username</label>
        <textarea class="form-control" id="username" rows="3"
            >{$user.username}</textarea
        >
    </div>
    <button type="submit" class="btn btn-primary">Edit Changes</button>
{/if}

{#if $isAuthenticated && $user.user_roles == "fitnessuser"}
    <h2>Fullfillment Account Details</h2>

    <div class="mb-3">
        <label for="email" class="form-label">Email address</label>
        <input
            type="email"
            class="form-control"
            placeholder={$user.email}
            readonly
        />
    </div>
    <div class="mb-3">
        <label for="username" class="form-label">Username</label>
        <textarea class="form-control" id="username" rows="3"
            >{$user.username}</textarea
        >
    </div>
    <div class="mb-3">
        <label for="birthDate" class="form-label">Birth Date</label>
        <input
            type="text"
            name="birthDate"
            class="form-control"
            id="birthDate"
            bind:value={fitnessuser.birthDate}
        />
    </div>
    <div class="mb-3">
        <label for="height" class="form-label">Height</label>
        <input type="text" name="height" class="form-control" id="height" bind:value={fitnessuser.height} />
    </div>
    <div class="mb-3">
        <label for="weight" class="form-label">Weight</label>
        <input type="text" name="weight" class="form-control" id="weight" bind:value={fitnessuser.weight} />
    </div>
    <button type="button" class="btn btn-primary" on:click={editUser}
        >Edit Changes
    </button>
{/if}
