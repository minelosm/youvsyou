...
public class FitnesscoachController {
    ...
    @PutMapping("/fitnesscoach/edit/{email}")
    public ResponseEntity<Fitnesscoach> putAccountDetails(
            @PathVariable String email,
            @RequestBody Fitnesscoach updatedUser) {
        return fitnesscoachRepository.findByEmail(email).map(fitnesscoach -> {
            if (updatedUser.getFitnesscenter() != null)
                fitnesscoach.setFitnesscenter(updatedUser.getFitnesscenter());

            fitnesscoachRepository.save(fitnesscoach);
            return new ResponseEntity<>(fitnesscoach, HttpStatus.OK);
        }).orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
}
